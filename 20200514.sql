...
아래 두개의 SQL을 서로다른 SQL로 인식한다(실행결과는 같음)
SELECT * FROM emp;
Select * FROM emp;
...
LHU.v_emp ==> v_emp 시노님으로 생성
CREATE SYNONYM v_emp FOR LHU.v_emp;

v_emp를 통해 실체 객체인 LHU.v_emp를 사용할 수 있다
...
Multiple insert : 여러건의 데이터를 동시에 입력하는 insert의 확정 구문
...
emp_test 테이블을 이용하여 emp_test2 테이블생성
...
SELECT 결과의 행의 값이 EMPNO = 99998이면 emp_test에만 데이터를 입력
                                    그외에는 emp_trest2에 데이터입력
                                    ;
INSERT ALL
...
emp 테이블에는 존재하고 emp_test테이블에는 존재하지 않는 직원을 신규 입력
emp 테이블에는 존재하고 emp_test테이블에도 존재하는 직원의 이름 변경;
...
9999번 사번으로 신규 입력하거나, 업데이트를 하고싶을때
...
emp 테이블을 이용하여 부서번호별 직원의 급여 합과, 전체직원의 급여 합을 조회하기 위해
