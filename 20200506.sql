...
크게 다음 두가지 형태로 사용
1. 테이블의 모든 컬럼에 값을 입력하는 경우, 컬럼명을 나열하지않아도 된다
   단 입력할 값의 순서는 테이블에 정의된 컬럼 순서로 인식 된다.
INSERT INTO 테이블명 VALUES (입력할 값, 입력할 값2 ....);

2. 입력하고자 하는 컬럼을 명시하는 경우
   사용자가 입력하고자 하는 컬럼을 선택 하여 데이터를 입력할 경우.
   단 테이블에  NOT NULL 설정이 되어있는 컬럼이 누락되면 INSERT는 실패한다.
INSERT INTO 테이블명 (컬럼1, 컬럼2) VALUES (입력할 값1, 입력할 값2);

3. SELECT 결과를 INSERT
   SELECT 쿼리를 이용해서 쿼리의 의해 조화되는 결과를 테이블에 입력 가능
   --> 여러건의 데이터를 하나의 쿼리로 입력 가능(ONE QUERY) --> 성능 개선

    사용자로부터 데이터를 직접 입력 받는 경우 ( ex 회원가입)는 적용이 불가
    db상에 존재하는 데이터를 갖고 조작하는 경우 활용가능(이런 경우가 많음)
...
데이터 입력를 확정 지으려면 : commit - 트랜잭션 완료
...
INSERT INTO emp (empno, ename, hiredate) VALUES (9999, '이 희욱', SYSDATE);
...
위에서 작업하누 INTSERT 구문을 기술
rollback;

UPDATE : 데이터 수정
UPDATE 테이블명 SET 수정할 컬럼1 = 수정할 값1,
                     [수정할 컬럼1 = 수정할 갑사, .....)]
WHERE conditgion - SELECT 절에서 배운 WHERE 절과 동일 
        수정할 행을 인식하는 조건을 기술)
...
deptno, job 두개의 컬럼을 SMITH 사원의 정보와 동일하게 변경
...
일반적인 UPDATE 구분에서는 컬럼별로 서브쿼리를 작성하여 비효율이 존재
...
트랜잭션 : 논리적인 일의 단위
ex : ATM - 출금과 입금이 둘다 정상적으로 이루어져야 문제가 발생되지 안흠
...
오라클에서는 첫번쨰 DML이 시작이 되면 트랜의 시작으로 인식
...
즉 두개중에 하나라도 문제가 생기면 일반적으로 ROLLBACK을 통해 두 개의 INSERT 구분을 취소.
